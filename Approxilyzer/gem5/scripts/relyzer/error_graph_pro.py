...
class bit_node(node):
	def __init__(self,name,in_string):
		...
		if in_string != '':
			...
			temp1 = in_string.split('::')[2]
			if(',' in temp1):
				self.str_ctl = temp1.split(','[0])
			else:
				self.str_ctl = temp1
	...
	def print_node(self):
		...
		return '%s %s %s %s %s %s %s %s %s %s %s %s' % (app_name+'_'+str(self.name), self.pc,isa,self.tick,self.opcode, self.reg, self.bit, self.reg_type, self.src_dest, str_ctl, inj,self.category)
...
class instn_database(node_database):
	...
	def load_opcode(self,bn_db):
                #overall_num = 0.0
                for instn in self.nodes:
                        for rn in instn.children:
                                for bn in rn.children:
                                        bn.opcode = instn.opcode
					#print(bn.opcode)
        	...
        	for instn in self.nodes:
            		...
            		for rn in instn.children:
                		for bn in rn.children:
                    			...
                    			elif(bn.category == 'SDC:Eggregious' or bn.category == 'SDC:Eggregious-line_num_mismatch'  or bn.category == 'SDC:Tolerable' or bn.category == 'SDC:Eggregious-pixel_mismatch'):
						#print(sdc_num)
                        			sdc_num = sdc_num +1
            		if(total_num !=0):
...
if len(sys.argv) < 3 or len(sys.argv) > 5:
	print ('Usage: python error_graph.py [app_name] [isa] [config_node_attrib]')
	exit()
app_name=sys.argv[1]
isa = sys.argv[2]
