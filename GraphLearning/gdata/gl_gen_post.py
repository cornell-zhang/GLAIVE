...
print(start_list)
sdc_dir = gl_dir+'/sdc_output/'
sdc_file=sdc_dir + test_app+ '_sdc_output'

start_flag=False
with open(sdc_file) as ff:
    for line in ff:
        id_test = int(line.split()[0])
        test_value = line.split()[2]
        if id_test in start_list and start_flag == False :
            ...
            sdc_post = sdc_dir + app_map[id_test] +'_post.txt'
	...
	#sdc_postfile.write(feats_all[id_test][5].strip()  + ',')
	#sdc_postfile.write(feats_all[id_test][6].strip()  + ',')
	#sdc_postfile.write(feats_all[id_test][7].strip()  + ',')
	for i in range(5,8):
